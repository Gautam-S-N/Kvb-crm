    ...
    <Layout>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Track monthly milestones and achievements</p>
        </div>
          ...
          return (
            <div key={t.id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 relative overflow-hidden">
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <h3 className="font-bold text-gray-900">{t.employee.firstName} {t.employee.lastName}</h3>
                  <p className="text-sm text-gray-500">{MONTHS[t.periodNumber - 1]} {t.periodYear} Target</p>
                </div>
              </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-100 flex justify-between">
              <h2 className="font-bold text-lg">Set Monthly Target</h2>
            ...
            <form onSubmit={handleCreate} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-semibold block mb-1">Month (1-12)</label>
                  <input type="number" min="1" max="12" required value={formData.periodNumber} onChange={e=>setFormData({...formData,periodNumber:+e.target.value})} className="w-full px-3 py-2 border rounded-lg text-sm" />
                </div>
                <div>
                  <label className="text-xs font-semibold block mb-1">Year</label>
                  <input type="number" required value={formData.periodYear} onChange={e=>setFormData({...formData,periodYear:+e.target.value})} className="w-full px-3 py-2 border rounded-lg text-sm" />
                </div>
              </div>
              <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input type="number" required value={formData.quotationsTarget} onChange={e=>setFormData({...formData,quotationsTarget:+e.target.value})} className="w-full px-3 py-2 border rounded-lg text-sm" />
                </div>
              </div>
