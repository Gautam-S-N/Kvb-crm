...
exports.getTasks = async (req, res) => {
  try {
    ...
    const shouldIncludeMreq = req.query.includeMaterialRequests === 'true' ||
                              req.path.includes('/tasks') ||
                              (!type || type === 'TEAM');

    if (shouldIncludeMreq) {
      ...
      if (assignedToId) {
        ...
        const validUserIds = await getSubordinateIds(userId, true);
        validUserIds.push(userId);
        mreqConditions.push(inArray(schema.materialRequests.assignedToId, validUserIds));
      }
      ...
      if (status) {
        if (status === 'COMPLETED') mreqConditions.push(eq(schema.materialRequests.status, 'COMPLETE'));
          ...
          mreqConditions.push(eq(schema.materialRequests.id, 'FORCE_EMPTY_RESULT'));
        }
      }
      ...
      if (search) {
        mreqConditions.push(
          or(
            like(schema.materialRequests.title, `%${search}%`),
            like(schema.materialRequests.projectName, `%${search}%`),
            like(schema.materialRequests.location, `%${search}%`)
          )
        );
      }
      ...
      const formattedMreqs = mreqsRows.map(m => ({
        ...
        assignedToId: m.assignedTo?.id || null,
        createdById: m.createdBy?.id || null,
        items: itemsMap[m.id] || [],
...
exports.getTaskById = async (req, res) => {
  try {
    ...
    if (tasksRows.length === 0) {
      // Check if it's a material request
      ...
      if (mrRows.length > 0) {
        ...
        // Fetch assignee and creator
        const assignedToUser = await db.select().from(schema.users).where(eq(schema.users.id, mr.assignedToId)).limit(1).then(r => r[0]);
        const createdByUser = await db.select().from(schema.users).where(eq(schema.users.id, mr.createdById)).limit(1).then(r => r[0]);
        
        // Fetch items
        const items = await db.select().from(schema.materialRequestItems).where(eq(schema.materialRequestItems.materialRequestId, mr.id));

        return res.json({
          success: true,
          data: {
            id: mr.id,
            title: `${mr.title} (Material Request)`,
            description: `${mr.projectName || ''} ${mr.location || ''} ${mr.notes || ''}`.trim(),
            status: mr.status === 'COMPLETE' ? 'COMPLETED' : mr.status === 'INCOMPLETE' ? 'CANCELLED' : mr.status,
            priority: 'MEDIUM',
            dueDate: mr.createdAt,
            assignedTo: assignedToUser ? { id: assignedToUser.id, firstName: assignedToUser.firstName, lastName: assignedToUser.lastName } : null,
            createdBy: createdByUser ? { id: createdByUser.id, firstName: createdByUser.firstName, lastName: createdByUser.lastName } : null,
            items: items,
            isMaterialRequest: true,
            type: 'TEAM'
          }
...
exports.completeTask = async (req, res) => {
  try {
    ...
    if (taskList.length === 0) {
      // Check if it's a material request!
      const mrList = await db.select()
        .from(schema.materialRequests)
        .where(eq(schema.materialRequests.id, id))
        .limit(1);
      ...
      if (mrList.length > 0) {
        ...
        const updateData = {
          ...
          completionNote: completionVoiceNote || null,
        ...
        const io = req.app.get('io');
        if (mr.createdById !== req.user.id) {
          const notifBody = completionVoiceUrl
            ? `"${mr.title}" marked complete. Voice note attached.`
            : `"${mr.title}" has been marked as completed`;
          ...
          if (io) {
            io.emit('notification', {
              ...
              entityId: id,
              targetUserId: mr.createdById
            });
          }

          await db.insert(schema.notifications).values({
            id: randomUUID(),
            userId: mr.createdById,
            type: 'TASK_COMPLETED',
            title: 'Task Completed',
            body: notifBody,
            entityType: 'material_request',
        ...
        if (io) {
          io.emit('REFRESH_DATA', { module: 'TASKS' });
          io.emit('REFRESH_DATA', { module: 'MATERIAL_REQUESTS' });
          io.emit('REFRESH_DATA', { module: 'DASHBOARD' });
        }
    ...
    if (taskList.length === 0) {
      ...
      if (mrList.length > 0) {
        ...
        const updateData = {
          ...
          failureVoiceUrl: completionVoiceUrl || null,
          updatedAt: new Date()
        };
        await db.update(schema.materialRequests)
          .set(updateData)
        ...
        const io = req.app.get('io');
        if (mr.createdById !== req.user.id) {
          const notifBody = `"${mr.title}" was marked incomplete. Reason: ${failureReason?.substring(0, 50) || 'None'}`;
