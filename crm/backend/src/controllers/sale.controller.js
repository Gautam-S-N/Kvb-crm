  ...
  try {
    ...
    try {
      ...
      try {
        ...
        await receiptPage.setContent(html, { waitUntil: 'networkidle0' });
        await receiptPage.pdf({ path: receiptPath, format: 'A5', printBackground: true });
      } finally {
        if (receiptBrowser) await receiptBrowser.close();
      }
...
exports.generateInvoice = async (req, res) => {
  try {
    ...
    let invoiceBrowser;
    try {
      invoiceBrowser = await puppeteer.launch({
        headless: 'new',
        args: [
          '--no-sandbox', '--disable-setuid-sandbox',
          '--disable-dev-shm-usage', '--disable-accelerated-2d-canvas',
      ...
      await invoicePage.setContent(html, { waitUntil: 'networkidle0' });
      await invoicePage.pdf({ path: invoicePath, format: 'A4', printBackground: true });
    } finally {
      if (invoiceBrowser) await invoiceBrowser.close();
    }
